Shader "Universal Render Pipeline/2D/TilemapCheckerboardLit"
{
    Properties
    {
        ...
        _DarkTint("Dark Tile Tint", Color) = (0.7, 0.72, 0.78, 1)
        _CellSize("Cell Size", Float) = 1.0
        _GridOffset("Grid Offset", Vector) = (0, 0, 0, 0)

        // Legacy properties for sprite shader fallback compatibility
    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            half4 LitFragment(Varyings input) : SV_Target
            {
                ...
                float cellSize = max(0.0001, _CellSize);
                float2 coord = floor((input.worldPos - _GridOffset.xy) / cellSize);
                float checker = fmod(abs(coord.x + coord.y), 2.0);

                if (checker >= 0.5)
                {
                    main.rgb *= _DarkTint.rgb;
                }
