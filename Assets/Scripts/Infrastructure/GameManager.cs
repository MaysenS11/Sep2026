...
public class GameManager : MonoBehaviour
{
    ...
    {
        if (!DungeonDictionary.TryGetValue(CurrentRoomIndex, out RoomData currentRoom)) return;
        ...
        if (!targetDoor.HasValue || !DungeonDictionary.TryGetValue(nextRoomIndex, out RoomData nextRoom))
        {
        ...
        void OnTransitionComplete()
        {
            ...
            if (player != null)
            {
                player.TeleportTo(targetDoor.Value);
            }

            Vector2Int newGrid = BoardCoordinate.WorldToGrid(targetDoor.Value);

            if (Board != null)
            {
                var playerOcc = Board.FindPlayer();
                if (playerOcc != null)
                {
                    Board.Move(playerOcc, newGrid);
                }
            }

            if (player != null && player.TryGetComponent<Presentation.Entities.PlayerTileObject>(out var playerTileObj))
            {
                playerTileObj.SnapToGrid(newGrid);
            }

            NotifyNewRoomEntered(nextRoom);
    ...
    public Vector3? GetEntryDoor(int roomIndex)
    {
        if (DungeonDictionary.TryGetValue(roomIndex, out RoomData room))
        {
            ...
            if (room.EntranceDoorTile.HasValue) return BoardCoordinate.GridToWorldCenter(room.EntranceDoorTile.Value);
            return BoardCoordinate.GridToWorldCenter(room.CenterTile);
        }
        return null;
    }

    public Vector3? GetExitDoor(int roomIndex)
    {
        if (DungeonDictionary.TryGetValue(roomIndex, out RoomData room))
        {
    ...
    public Vector3? GetSpecialEntryDoor(int roomIndex)
    {
        if (DungeonDictionary.TryGetValue(roomIndex, out RoomData room))
        {
    ...
    public Vector3? GetSpecialExitDoor(int roomIndex)
    {
        if (DungeonDictionary.TryGetValue(roomIndex, out RoomData room))
        {
